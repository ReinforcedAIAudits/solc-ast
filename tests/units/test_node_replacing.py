...
BASE_CONTRACT_PATH = join(dirname(__file__), "..", "examples", "replacing", "base")
EXPECTED_CONTRACT_PATH = join(
    dirname(__file__), "..", "examples", "replacing", "expected"
)
...
class ContractToAstTestCase(unittest.TestCase):
    ...
    def _test_single_contract(self, base_contract_filename, base_contract_name):
        ...
        intialize_call = find_node_with_properties(
            ...
            expression=find_node_with_properties(
                ast,
        ...
        self.assertEqual(
            len(intialize_call),
            1,
            f"Expected one 'initialize' function call in {base_contract_name}",
        )
        ...
        result = replace_node_to_multiple(
            ast,
            initialize_expression[0].id,
            intialize_definition[0].body,
        )
